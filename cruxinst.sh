...
prt-get install base linux linux-firmware base-devel ${CPU_UCODE} \
...
prt-get install ${DE_PACKAGES}

# 해당되는 경우 디스플레이 매니저 활성화
if [[ $DM_SERVICE != "none" ]]; then
    ln -s /etc/sv/${DM_SERVICE} /mnt/etc/runit/runsvdir/default/
fi

# 네트워크 활성화
ln -s /etc/sv/NetworkManager /mnt/etc/runit/runsvdir/default/

# 시스템 구성
chroot /mnt /bin/bash <<CHROOT_COMMANDS
# 시간대를 대한민국으로 설정
